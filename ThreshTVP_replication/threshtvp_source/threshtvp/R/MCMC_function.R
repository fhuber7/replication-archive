...
# --- compatibility shim for stochvol (svsample2 was deprecated) -----------
# Performs ONE Gibbs step of the SV model. Returns a plain list with the
# fields used downstream (`para`, `latent`) so the rest of the sampler
# does not have to care about stochvol's object model.
.sv_step <- function(u_, svdraw,
                     priorphi = c(2, 2),
                     priormu  = c(0, 100)) {
  if (!requireNamespace("stochvol", quietly = TRUE)) {
    stop("Package 'stochvol' is required.")
  }
  ps <- svdraw$para
  pl <- svdraw$latent
  res <- stochvol::svsample_fast_cpp(
    as.numeric(u_),
    startpara  = list(mu = unname(ps["mu"]),
                      phi = unname(ps["phi"]),
                      sigma = unname(ps["sigma"]),
                      nu = Inf, rho = 0, beta = 0, latent0 = 0),
    startlatent = as.numeric(pl),
    priorspec  = stochvol::specify_priors(
      mu    = stochvol::sv_normal(mean = priormu[1], sd = sqrt(priormu[2])),
      phi   = stochvol::sv_beta(shape1 = priorphi[1], shape2 = priorphi[2]),
      sigma2 = stochvol::sv_gamma(shape = 0.5, rate = 0.5)
    )
  )
  list(
    para   = c(mu    = as.numeric(res$para[1, "mu"]),
               phi   = as.numeric(res$para[1, "phi"]),
               sigma = as.numeric(res$para[1, "sigma"])),
    latent = as.numeric(res$latent[1, ])
  )
}
...
MCMC_tvp <- function(Y,X,nburn,nsave,priorbtheta=list(B_1=2,B_2=1,kappa0=1e-07),priorb0=list(a_tau=0.1,c_tau=0.01,d_tau=0.01),priorsig=c(0.01,0.01),priorphi=c(2,2),priormu=c(0,10^2),h0prior="stationary",grid.length=150,thrsh.pct=0.1,thrsh.pct.high=1,sv_on=TRUE,TVS=TRUE,cons.mod=TRUE,nr,thin=0.1,robust=TRUE,a.approx=FALSE, sim.kappa=FALSE,kappa.grid = seq(1e-4, 0.1, 10)){
  ...
  #mylm <- lm(Y__ ~ X__)
  XXinv <- MASS::ginv(crossprod(X__))
  B.OLS <- XXinv%*%crossprod(X__,Y__)
  sig.OLS <- crossprod(Y__-X__%*%B.OLS)/(T-K_)
  V.OLS <- as.numeric(sig.OLS)*XXinv
  ...
  ALPHA0 <- XXinv%*%crossprod(X__,Y__)
  ...
  A_store <- array(NA,c(thin*nsave,T,K_))
  ...
  for (irep in 1:ntot){
    ...
    if (sv_on){
      ...
      svdraw <- .sv_step(u_, svdraw, priorphi = priorphi, priormu = priormu)
      hv <- svdraw$latent
      sig_eta <- 0
    }else{
      S_1 <- a_sig+T/2
    ...
    if (irep %in% thin_out){
      ...
      svparms_store[ithin,] <- svdraw$para
      D_store[ithin,,] <- D_t
  ...
  return(list(A=ALPHA_store,V0=V0_store,D_dyn=D_store,H=H_store,svparms=svparms_store,threshold.prior=thrshprior_store, A.sparse=A_store,
