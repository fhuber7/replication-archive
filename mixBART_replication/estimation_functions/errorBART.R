# ------------------------------------------------------------------------------------
# MCMC setup
errorBART <- function(Yraw, nburn=5000, nsave = 5000, thinfac = 1, prior="HS", prior.sig, sv = "SV", fc.approx="exact", fhorz = 3, quiet = FALSE){
  # auxiliary functions
  source("aux_func.R")
  ...
  # initialize draws
  PHI_draw  <- A_draw <- A_approx <- A_OLS <- solve(crossprod(X)+diag(ncol(X))*100)%*%crossprod(X,Y)
  ...
  shock.lags <- mlag(E.hat, p)
  for (t in seq_len(p)) shock.lags[t,] <- rnorm(M)%*%t(chol(Sig_OLS))
  ...
  for (irep in 1:ntot){
    ...
    Y__ <- Y - tcrossprod(Ft,Lambda) - X%*%PHI_draw
    while (check & try.count <= max.tries){
      try.count <- try.count + 1
      for (mm in seq_len(M)){
        sampler.list[[mm]]$setResponse(Y__[,mm])
        sampler.list[[mm]]$setPredictor(shock.lags)
        ...
        A_approx[,mm] <- ginv(shock.lags)%*%rep_mm$train
        ...
        if (is(mu.phi.a0.draw, "try-error")){
          mu.phi.a0.draw <-  as.numeric(mvtnorm::rmvnorm(1, mu.cov, (V.cov)))
        } 
        error.full[,mm] <- eta[,mm]
        PHI_draw[ , mm] <- mu.phi.a0.draw
      }
      
      A_draw <- matrix(0, K, M)
      max.eigen <- (max(abs(eigen(get_companion(PHI_draw, c(M, 0, p))$MM)$values)))
      if (irep > 0.05*nburn){
        if (max.eigen < 1.2) check <- FALSE
      }else{
        check <- FALSE
      }
    }
    if(try.count == max.tries){
      message(paste0("\n No stable draw, Eigen = ",round(max.eigen,digits=3)))
    }
    
    Y.fit.VAR <- X%*%PHI_draw
    Y.fit.full <- Y.fit.BART + Y.fit.VAR
    
    # Update shocks based on the previous draw of PHI
    E.hat <- Y - X%*%PHI_draw
    shock.lags <- mlag(E.hat, p)
    
    for (t in seq_len(p)){
      S.t <- Sig_t[tt,,]
      shock.lags[t,] <- rnorm(M)%*%t(chol(S.t))
    } 
    ...
    if(sv=="SV"){
      ...
      for(qq in 1:Q){
        Omega[,qq] <- 1 # log(1/rgamma(1,3+T/2,0.3+sum(Ft[,qq]^2)))
      }
    }
    ...
    if(irep %in% thin.set){
      ...
      if(fhorz>0){
        ...
        u.hat <- c(E.hat[T, ],shock.lags[T,1:(M*(p-1))])
        ...
        for (hh in seq_len(fhorz)){
          if (sv == "SV"){
            ...
            #   HT <- as.numeric(log(as.numeric(sigma.mat)^2) + as.numeric(c(T+hh, X.hat)%*%ASV_approx))
            #   OT <- as.numeric(log(as.numeric(sigma.fac.mat)^2) + as.numeric(c(T+hh, X.hat)%*%AFSV_approx))
            # }
          }
          ...
          for (j in seq_len(M)) tree.pred[j] <- sampler.list[[j]]$predict(u.hat)
          if(fc.approx=="approx"){
            tree.pred <- as.numeric(u.hat%*%A_approx)
          }
          ...
          u.hat <- c(as.numeric(tree.pred), u.hat[1:(M*(p-1))])
          Yfc[hh,] <- Y.tp1
        }
